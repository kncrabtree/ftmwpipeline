...
def cross_interface_stage1_trio(
    exp_2638_data_path, tmp_path_factory, standard_ft_params
):
    """
...
def cli_helper():
    ...
    class CLIHelper:
        ...
        @staticmethod
        def run_command(
            args: List[str], check_return_code: bool = True
        ) -> Tuple[int, str, str]:
            """
        ...
        @staticmethod
        def visualize_ft(
            ftmw_file: Path, save_params: bool = False, **params
        ) -> Tuple[int, str, str]:
            """Run ft show command with parameters."""
            args = ["ft", "show", str(ftmw_file)]
...
def interface_results_validator():
    ...
    class InterfaceResultsValidator:
        ...
        @staticmethod
        def compare_complex_ft(
            ft1: ComplexFT, ft2: ComplexFT, rtol: float = 1e-10
        ) -> None:
            """
        ...
        def compare_pipeline_states(
            info1: Dict[str, Any], info2: Dict[str, Any]
        ) -> None:
            """
            ...
            assert (
                info1["source_metadata"] == info2["source_metadata"]
            ), "Source metadata differs between interfaces"

            # Stage completion status should match
            assert (
                info1["stage_tracker"] == info2["stage_tracker"]
            ...
            if "processing_parameters" in info1 and "processing_parameters" in info2:
                assert (
                    info1["processing_parameters"] == info2["processing_parameters"]
...
def cli_command_runner():
    ...
    class CLICommandRunner:
        ...
        @staticmethod
        def run_command(
            args: List[str], check_success: bool = True, timeout: int = 30
        ) -> Tuple[bool, str, str]:
            """
            Run ftmwpipeline CLI command and capture output.
        ...
        @staticmethod
        def import_data(
            ftmw_file: Path, source: str, **kwargs
        ) -> Tuple[bool, str, str]:
            """Run data import command."""
            args = ["data", "import", str(ftmw_file), source]
