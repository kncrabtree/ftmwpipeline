...
class TestPipelineClassWorkflows:
    ...
    def test_pipeline_class_basic_workflow(
        ...
        ), "compute_ft() should return ComplexFT object"
        assert complex_ft.n_points > 0, "ComplexFT should have frequency points"
        assert (
            len(complex_ft.complex_spectrum) == complex_ft.n_points
        ), "Complex spectrum length should match n_points"
    ...
    def test_pipeline_class_file_validation(self, exp_2638_data_path, temp_ftmw_file):
        ...
        assert validation_report[
            "valid"
        ], f"Validation failed: {validation_report.get('errors', 'Unknown')}"
        assert (
            len(validation_report.get("errors", [])) == 0
...
class TestFunctionalAPIWorkflows:
    ...
    def test_functional_api_basic_workflow(
        self, exp_2638_data_path, temp_ftmw_file, standard_ft_params
    ):
        ...
        assert result["pipeline_file"] == str(
            temp_ftmw_file
        ), "Wrong pipeline file path"
    ...
    def test_functional_api_parameter_persistence(
        self, exp_2638_data_path, temp_ftmw_file, standard_ft_params
    ):
        ...
        # Save parameters during visualization
        ftmw.visualize_ft(
            temp_ftmw_file, save_params=True, interactive=False, **standard_ft_params
        )
...
class TestCLIWorkflows:
    ...
    def test_cli_basic_workflow(
        ...
        success, stdout, stderr = self.run_cli_command(
            ["data", "import", str(temp_ftmw_file), exp_2638_data_path]
        )
        assert success, f"data import failed: {stderr}"
        assert temp_ftmw_file.exists(), "Pipeline file not created by CLI"
        assert (
            "Data import completed successfully" in stdout
        ...
        assert success, f"ft run failed: {stderr}"
        assert (
        ...
        assert info[
            "valid"
        ], f"CLI-created pipeline invalid: {info.get('errors', 'Unknown')}"
    ...
    def test_cli_file_creation_only(self, exp_2638_data_path, temp_ftmw_file):
        ...
        assert isinstance(
            fid, FID
        ), "CLI-created file should be readable by functional API"
        ...
        assert info[
            "valid"
        ], f"CLI-created file should be valid for Pipeline class: {info.get('errors', 'Unknown')}"

    def test_cli_error_handling(self, temp_ftmw_file):
        """Test CLI error handling for missing files and invalid parameters."""
        # Test data import with non-existent source
        success, stdout, stderr = self.run_cli_command(
            ["data", "import", str(temp_ftmw_file), "nonexistent_source"],
            check_return_code=False,
        )
        assert not success, "data import should fail with non-existent source"

        # Test ft run with non-existent file
        success, stdout, stderr = self.run_cli_command(
            ["ft", "run", "nonexistent_file.ftmw"], check_return_code=False
        )
        assert not success, "ft run should fail with non-existent file"

        # Test invalid trim range
        success, stdout, stderr = self.run_cli_command(
            ["ft", "run", str(temp_ftmw_file), "--trim", "invalid_range"],
            check_return_code=False,
        )
        assert not success, "ft run should fail with invalid trim range"
    ...
    def test_cli_validation_commands(self):
        ...
        success, stdout, stderr = self.run_cli_command(
            ["validate"], check_return_code=False
        )
        # Command should run and produce output even if some components fail
        assert (
            "ftmwpipeline installation validation" in stdout
