...
class TestStage0FIDCaching:
    ...
    def test_fid_cache_roundtrip_with_real_data(self):
        ...
        assert cached_fid.probe_freq_mhz == original_fid.probe_freq_mhz, "Probe frequency not preserved"
        ...
        if original_fid.processing is not None:
            assert cached_fid.processing is not None, "Processing parameters not preserved"
            original_params = original_fid.processing
            cached_params = cached_fid.processing
            
            assert cached_params.zpf == original_params.zpf, "Zero padding factor not preserved"
            assert cached_params.start_us == original_params.start_us, "Start time not preserved"
            assert cached_params.end_us == original_params.end_us, "End time not preserved"
            assert cached_params.expf_us == original_params.expf_us, "Exponential filter not preserved"
        
        # Metadata should preserve original experiment information
        assert cached_fid.metadata['experiment_path'] == str(self.example_data_path), "Experiment path not preserved"
    ...
    def test_processing_parameter_updates(self):
        ...
        if not self.has_example_data:
            pytest.skip("Experiment 2638 data not available for parameter update testing")
...
class TestStage1OnDemandComplexFT:
    ...
    def test_on_demand_complexft_consistency(self):
        ...
        if not self.has_example_data:
            pytest.skip("Experiment 2638 data not available for ComplexFT consistency testing")
    ...
    def test_parameter_variation_produces_different_results(self):
        ...
        if not self.has_example_data:
            pytest.skip("Experiment 2638 data not available for parameter variation testing")
...
class TestStage01WorkflowIntegration:
    ...
    def test_full_stage01_pipeline_vs_direct_processing(self):
        ...
        print(f"Storage overhead: {(cache_file_size - fid_data_size):.2f} MB (metadata)")
        print(f"ComplexFT size (temporary): {complex_ft_direct.complex_spectrum.nbytes / 1024**2:.2f} MB")
        print(f"Storage efficiency: FID cached once, unlimited ComplexFT parameter combinations")
        print("================================================================")
    ...
    def test_stage01_with_multiple_parameter_exploration(self):
        ...
        if not self.has_example_data:
            pytest.skip("Experiment 2638 data not available for parameter exploration testing")
        ...
        print(f"Single FID cache supports {len(exploration_params)} parameter combinations")
...
class TestStage01CacheRobustness:
    ...
    def test_corrupted_fid_cache_handling(self):
        ...
        with pytest.raises((OSError, RuntimeError, FileNotFoundError)):
            load_fid_cache(self._get_cache_id("corrupted_fid"), str(self.test_output_dir))
    ...
    def test_missing_fid_cache_components(self):
        ...
        with pytest.raises((ValueError, KeyError, FileNotFoundError)):
            load_fid_cache(self._get_cache_id("incomplete_fid"), str(self.test_output_dir))
