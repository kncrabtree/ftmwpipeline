...
class TestInitialBuild:
    def test_stage2_group_present_and_completed(self, exp_2638_data_path, tmp_path):
        ...
        assert "stage2_noise_result" in _completed(
            fp
...
class TestIdempotentRepersist:
    def test_same_settings_leave_stage2_intact(
...
class TestChangedSettingsInvalidatesStage2:
    def test_changed_start_us_invalidates_stage2(
        ...
        assert "stage2_noise_result" not in _completed(
            fp
    ...
    def test_changed_trim_invalidates_stage2(
        self, exp_2638_data_path, tmp_path, caplog
    ):
...
class TestChangedSettingsInvalidatesChain:
    def test_both_stage2_and_stage3_invalidated(
        self, exp_2638_data_path, tmp_path, caplog
    ):
        ...
        assert _has_group(
            fp, "stage3_peaks"
        ), "Stage 3 not persisted after detect_peaks"
        ...
        assert (
            invalidation_warnings
        ), "No invalidation warning for full-chain invalidation"
