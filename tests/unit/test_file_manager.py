...
class TestPipelineStageTracker:
    ...
    def test_stage_tracker_serialization(self):
        ...
        assert set(data_dict['completed_stages']) == {'stage0_fid_data', 'stage1_complex_ft'}
...
class TestFileManagerFunctions:
    ...
    def sample_fid(self):
        ...
        data = np.exp(-t / 2e-6) * np.cos(2 * np.pi * 100e6 * t) + 0.01 * np.random.normal(0, 1, n_points)
    ...
    def test_create_pipeline_file_functional(self, temp_dir, sample_fid, sample_source_metadata):
    ...
    def test_create_pipeline_file_extension_handling(self, temp_dir, sample_fid, sample_source_metadata):
        """Test that .ftmw extension is added automatically for user convenience."""
        filepath_no_ext = temp_dir / "test_pipeline"
        
        result_path = create_pipeline_file(filepath_no_ext, sample_fid, sample_source_metadata)
    ...
    def test_open_pipeline_file_functional(self, temp_dir, sample_fid, sample_source_metadata):
    ...
    def test_open_pipeline_file_error_handling(self, temp_dir):
        ...
        assert "ftmwpipeline import-data" in error_message  # Should suggest CLI alternative
    
    def test_validate_pipeline_file_functional(self, temp_dir, sample_fid, sample_source_metadata):
    ...
    def test_update_processing_parameters_functional(self, temp_dir, sample_fid, sample_source_metadata):
    ...
    def test_update_processing_parameters_with_none_values(self, temp_dir, sample_fid, sample_source_metadata):
...
class TestRealDataIntegration:
    ...
    def test_experiment_2638_end_to_end_workflow(self, temp_dir):
        ...
        try:
            ...
            # 2. Open and verify
            opened_path, loaded_metadata, stage_tracker = open_pipeline_file(created_path)
...
class TestCustomExceptions:
    ...
    def test_pipeline_corruption_error_suggests_recovery(self):
        ...
        assert "recreating" in error_str.lower()  # Should suggest recreating from source