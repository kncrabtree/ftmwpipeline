...
from ftmwpipeline.io.data_loaders.registry import FormatRegistry, register_loader, detect_format
...
class TestBlackChirpLoader:
    ...
    def sample_blackchirp_dir(self):
        ...
        fid_data = ['0', '1k', '2z', 'a0', 'z1', '10', '0', '-1k', '-2z']  # Sample base-36 data
        fid_df = pd.DataFrame({'data': fid_data})
        fid_df.to_csv(fid_dir / "0.csv", index=False)
        
        # Create optional processing.csv
        processing_data = {
            'ObjKey': ['FidStartUs', 'FidEndUs', 'FidZeroPadFactor', 'FidExpfUs', 'FidRemoveDC'],
...
class TestExperiment2638Integration:
    ...
    def test_load_real_experiment_2638(self):
        ...
        assert validation['metadata']['sideband'] == 'LowerSideband'  # Actual value in experiment 2638 
...
class TestFormatRegistry:
    ...
    def test_register_new_loader(self):
        ...
        class MockLoader(BaseLoader):
            ...
            def can_load(self, source_path): return True
            def validate_source(self, source_path, **kwargs): return {'valid': True}
            def load_fid(self, source_path, **kwargs): pass
            def get_required_parameters(self): return []
            def get_optional_parameters(self): return {}
    ...
    def test_global_register_function(self):
        ...
        class TestGlobalLoader(BaseLoader):
            ...
            def can_load(self, source_path): return False
            def validate_source(self, source_path, **kwargs): return {'valid': False}
            def load_fid(self, source_path, **kwargs): pass
            def get_required_parameters(self): return []
            def get_optional_parameters(self): return {}
...
class TestBaseLoaderErrorHandling:
    ...
    def test_invalid_loader_registration(self):
        ...
        with pytest.raises(ValueError, match="loader_class must inherit from BaseLoader"):
            registry.register_loader("invalid_format", str)  # str is not a BaseLoader subclass
