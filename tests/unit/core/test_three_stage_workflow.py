...
from ftmwpipeline.core.data_structures import (
    FID, PreprocessedFID, ComplexFT, FIDProcessingParameters, Sideband
)
...
class TestPreprocessedFID:
    ...
    def test_windowing_bounds(self, sample_fid):
        ...
        preprocessed = sample_fid.preprocess(start_us=duration_us * 0.1, end_us=duration_us * 0.9)
        assert isinstance(preprocessed, PreprocessedFID)
        ...
        preprocessed = sample_fid.preprocess(start_us=duration_us * 0.1, end_us=duration_us)
        assert isinstance(preprocessed, PreprocessedFID)
    ...
    def test_zero_padding_factor(self, sample_fid):
        ...
        expected_1 = 2 ** (int(np.log2(original_length)) + 1 + 1)  # Next power of 2, then doubled
        ...
        expected_2 = 2 ** (int(np.log2(original_length)) + 1 + 2)  # Next power of 2, then quadrupled
        ...
        assert preprocessed_1.n_points > preprocessed_0.n_points  # zpf=1 should be larger than zpf=0
    
    def test_exponential_filtering(self, sample_fid):
        """Test exponential filtering application."""
        # Test without filtering
        duration_us = sample_fid.duration_us
        no_filter = sample_fid.preprocess(expf_us=None, start_us=duration_us * 0.1, end_us=duration_us * 0.9)
        
        # Test with filtering
        with_filter = sample_fid.preprocess(expf_us=duration_us * 0.1, start_us=duration_us * 0.1, end_us=duration_us * 0.9)
    ...
    def test_dc_removal(self, sample_fid):
        ...
        with_rdc = fid_with_dc.preprocess(rdc=True, start_us=duration_us * 0.1, end_us=duration_us * 0.9)
        
        # Test without DC removal
        without_rdc = fid_with_dc.preprocess(rdc=False, start_us=duration_us * 0.1, end_us=duration_us * 0.9)
...
class TestPreprocessedFIDFFTComputation:
    ...
    def test_fft_normalization_and_scaling(self, sample_fid):
        ...
        preprocessed = sample_fid.preprocess(
            zpf=1,  # Double length
            units_power=6,  # Scale by 10^6 (μV)
            rdc=True
        )
...
class TestThreeStageWorkflow:
    ...
    def sample_fid(self):
        ...
        data = (np.exp(-t / 2e-6) * 
                (1.0 * np.cos(2 * np.pi * 120e6 * t) +  # Strong 120 MHz line
                 ...
                 0.05 * np.random.normal(0, 0.1, n_points)))  # Noise
    ...
    def test_workflow_reproducibility(self, sample_fid):
        ...
        np.testing.assert_array_equal(
            preprocessed1.data, preprocessed2.data,
            err_msg="Preprocessing should be reproducible"
        )
        np.testing.assert_array_equal(
            spectrum1, spectrum2,
            err_msg="FFT results should be reproducible"
        )
        np.testing.assert_array_equal(
            freqs1, freqs2, 
            err_msg="Frequency arrays should be reproducible"
        )
    ...
    def test_workflow_parameter_independence(self, sample_fid):
        ...
        duration_us = sample_fid.duration_us
        params1 = {'zpf': 1, 'expf_us': duration_us * 0.1, 'start_us': duration_us * 0.1, 'end_us': duration_us * 0.8}
        params2 = {'zpf': 2, 'expf_us': duration_us * 0.2, 'start_us': duration_us * 0.05, 'end_us': duration_us * 0.9}
        ...
        assert not np.array_equal(spectrum1[:min(len(spectrum1), len(spectrum2))], 
...
class TestThreeStageWorkflowWithRealData:
    ...
    def test_experiment_2638_three_stage_workflow(self):
        ...
        try:
            ...
            assert complex_ft.freq_array[0] > 20000  # Should start above 20 GHz (lower sideband)
            assert complex_ft.freq_array[-1] < 41000  # Should end below 41 GHz
            assert len(complex_ft.complex_spectrum) > 500000  # Should have high resolution
            
            # Test trimming to activity region per CLAUDE.md recommendations
            trimmed_ft = complex_ft.trim_to_range(26500, 40000)  # Focus on activity region
            
            print(f"  Trimmed to activity region: {len(trimmed_ft.freq_array)} points")
            print(f"  Activity range: {trimmed_ft.freq_array[0]:.1f} to {trimmed_ft.freq_array[-1]:.1f} MHz")
            ...
            pytest.skip(f"Could not test three-stage workflow with experiment 2638 data: {e}")
    ...
    def test_three_stage_workflow_enforced(self):
        ...
        try:
            ...
            assert not hasattr(fid, 'ft'), "Legacy FID.ft() method should be removed to enforce three-stage workflow"
            ...
            print("✓ Three-stage workflow properly enforced - legacy FID.ft() method removed")
...
class TestWorkflowErrorHandling:
    ...
    def test_invalid_window_function(self):
        ...
        with pytest.raises(ValueError):  # scipy.signal.get_window should raise ValueError
