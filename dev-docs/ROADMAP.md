...
| [`planning/stage5-spur-masking.md`](planning/stage5-spur-masking.md) | Planning — detect clock/LO spurs (integer-MHz ∧ temporal-persistence joint gate, promoting the Stage 2b STFT `spur_clusters` catalogue) and keep them out of the Stage 5 fit: cluster residual/χ² mask + peak-nomination exclusion + optional spur-only window drop. Prototype + fingerprint validation in [`research/stage5-gaussian-audit/report.md`](research/stage5-gaussian-audit/report.md) § "Spur-detection prototype" (~98–103 of the ~106 excess-χ²ᵣ spur bucket recovered at ±2–3 bin masking; zero real-line false positives). Sibling structural item from the same audit — shape escalation (per-peak τ vs Voigt) — was **parked**; see the report's § "Decision: park shape escalation" |
