...
| Document | Status |
|---|---|
| [`planning/stage5-inline-cleanup.md`](planning/stage5-inline-cleanup.md) | **Spec (implementation deferred)** — fold the per-window Stage-5 cleanup (SNR-survival prune + VIF degenerate-collapse) out of the post-walk global pass and into the NLS worker's tail, so each node is fully clean before the DAG releases its dependents. Fixes the `#3b` stale-frozen-background class (dependents fit against pre-cleanup sources → don't reproduce; surfaced by the cascade gate, `stage6-cascade-refit.md`) and removes the global-kwargs re-resolution bug class (the in-walk cleanup inherits the node's exact `fw_kwargs`, incl. the per-band τ anchor). Decomposition: new outcome-native `refit_outcome` (in `plan_execution`, reuses the live frozen background, returns a `WindowOutcome` — no snapshot round-trip), outcome-level cleanup metrics (avoid the `FittingResult` conversion per iteration), a `finalize_node` callback injected into the walk (keeps `plan_execution` import-clean), frozen-background contributor **dedup** (a cleaned source with fewer lines must not double its skirt = the minimal window-level resolution), and moving `refit_window_core` down to `stage5_impl` (Stage 6 pulls it in). Re-baselines the 7 fixtures; the parallelization history explains why it isn't already there (no structural blocker). Does **not** replace the post-fit user-edit cascade (`§§C/D`) |
