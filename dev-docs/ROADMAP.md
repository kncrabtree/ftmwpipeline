...
2. **Fork once Stage 6 stabilizes — report generation favored over the
   ...
   - **Reports from the `.ftmw` record** (core; **shipped**, planned in
     ...
     the top-level **`report`** object is built across all three levels
     (assemble-once / render-many over the persisted record, dual-interface):
     `report table` (CSV + JSON + LaTeX), `report summary` (Markdown
     methods+results), `report full` (dependency-light linked-HTML per-window
     site). **Catalog cross-reference + σ_f pull calibration shipped** (`--catalog`
     ...
     Report UX round also shipped: an index full-spectrum overview + interactive
     SVG window-map navigation (clickable per-window bars, hover-zoom popup,
     replicated on every window page with a "you are here" highlight),
     auto-merge (VIF-collapse) fit-history provenance, MathJax-rendered methods
     equations with interleaved distribution histograms, a responsive
     stylesheet, and a spine-free / light-grid restyle of all spectral plots
     (report + `fit show`). The σ_f budget is diagnosed and settled: the
