...
1. **Complete Stage 6 — covariance, peak-survival metrics, window construction,
   ...
   2. **Devise variance–covariance peak-survival metrics, and iterate**
      ...
      an end-of-Stage-5 automatic pass — an absolute SNR floor (~3.2) prunes
      dust (F5), and a diagonal **amplitude VIF** `(amp_err/amp)·snr` with a
      sub-half-resolution separation guard collapses degenerate overfit (F4).
      Calibration measured on the persisted covariance: the off-diagonal
      *correlation* is **not** the discriminant (it saturates near `|1|` for
      every close pair); the diagonal VIF is, and the collapse must override
      χ²/AICc (fooled by the high-SNR lineshape floor). Truth set
      w217/w281/w161/w108/w419 + w438/w24; iterative, with a 7-fixture
      re-baseline. Phase A (SNR prune) then Phase B (VIF collapse).
