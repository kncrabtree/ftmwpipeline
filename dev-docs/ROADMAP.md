...
| [`planning/stage5-leakage-wing-baseline.md`](planning/stage5-leakage-wing-baseline.md) | Planning — evidence-triggered low-order complex **baseline** nuisance term per Stage 5 window, to absorb the coherent residual a neighbouring strong line's mismodeled leakage wing leaves behind (serves per-line extraction with fair uncertainties, not global consistency). Trigger empirically settled: `residual_edge_coherence > ~3.5` (vs thaw's 8.0) — recall 0.89, **zero harmful fires** on 2638; beats the coarse "fixed-contributor present" rule, which misses the biggest wins (w152/w150 have no contributor). 35 beneficial windows; chain w224–227/w309/w367 → χ²ᵣ≈1.5, σ_A inflation ≈×1.01, weak-line intensities de-biased. Supersedes the freeze-eligibility-guard framing (O4-2 / threads #6 + #11): the guard was falsified (w223's contributor is tightly pinned despite χ²ᵣ=202 — χ²ᵣ/σ anti-correlated), as was a physically-consistent skirt relaxation (no slack for a strong line). Threshold + order are calibration debt |
