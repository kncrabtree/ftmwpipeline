...
| [`planning/stage5-gaussian-shape.md`](planning/stage5-gaussian-shape.md) | Planning — add a Gaussian time-domain envelope as an alternative Stage 5 line shape via a `PeakShape` selector (LORENTZIAN \| GAUSSIAN). Covers shape-aware `h_T` / Jacobian, `window_fit` refactor, a Stage 2b twin `stage2b_tau_G_calibration` (lifting Part B's per-bin Voigt fit), and a Gaussian-vs-Lorentzian comparison harness. Motivated by [`planning/stage5-voigt-deficit.md`](planning/stage5-voigt-deficit.md)'s finding that the Voigt joint LSQ degenerates to Gaussian-dominant on 2638 |
