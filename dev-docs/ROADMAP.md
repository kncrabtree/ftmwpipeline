...
2. **Fork once Stage 6 stabilizes — report generation favored over the
   ...
   - **Reports from the `.ftmw` record** (core; in progress, planned in
     [`planning/stage6-reports.md`](planning/stage6-reports.md)): timebase-
     corrected frequencies + the σ_f budget, decision/candidate ledgers, spur
     provenance, catalog-match summaries; promotes the scratch validation harness
     into the user-facing pull calibration surface. **Step 1 (the persisted
     `FinalProducts` contract reports render) is done;** the remaining work is the
     top-level **`report`** object — three levels built L1→L2→L3: `report table`
     (CSV + JSON + LaTeX), `report summary` (Markdown methods+results), `report
     full` (dependency-light linked-HTML per-window site), assemble-once /
     render-many over the persisted record (§C). The σ_f budget is diagnosed
     and settled: the "overly optimistic" σ_f was not a covariance/χ² bug but an
     uncorrected unlocked-digitizer-clock bias (ε ≈ 2 ppm, `Δf = ε·f_baseband`),
     recovered prior-free from the clock spurs (`calibrate_timebase`). **Design
     decision: report precision only** — ship `sqrt(σ_stat² + (σ_ε·f_baseband)²
     ...
     it), so we do not bake it in. Pull ≈ unit normal is a user-calibration tool,
     not a shipped gate.
