...
A **proposed** next track is the **Stage 6 contributor-edit cascade**
([`planning/stage6-cascade-refit.md`](planning/stage6-cascade-refit.md)):
editing a contributor line does not propagate to the windows whose frozen skirt
depends on it, leaving the `.ftmw` internally inconsistent (and the documented
"Stage 6 reports which neighbors reference the window" claim is not actually
implemented — a divergence the work resolves). The cascade design is recorded but is now **gated on a prerequisite
dependency-model investigation**: characterizing the fixtures surfaced that the
dependency model is inconsistent across SNR — the extreme-SNR fixtures
(655/1512/1019), which have the most contributor coupling, have their *entire*
cyclic dependency graph dropped by the cycle-breaker (655: 9 523 edges dropped,
all 8 063 contributors demoted to edge-free, 0 surviving edges) and are fit as
independent windows, while moderate fixtures keep the edge-bearing machinery;
thaw is dormant (0 accepts anywhere). The suspected latent pathology is the
leakage-wing/complex baseline silently soaking up skirts that should be modeled
as contributors. Investigate first (reports + cycle-breaker/baseline
quantification on the dense fixtures), then decide between better cyclic
resolution and declaring dependencies unnecessary — made consistent across SNR —
before the cascade (which propagates along that graph) is built.

The active track is the **attention-metric refinements + coupled Stage 5 fitting
