...
## Cross-fixture knob audit (issue #3)

The 2638-tuned Stage 5 constants were validated across all seven persisted
fixtures (read-only roll-up over the bounded-merge builds; driver
`scratch/issue3_audit/audit_knobs.py`). Scope was *validate & document* — keep
the 2638 default unless a fixture clearly breaks it. **No default needed
changing.** Per knob:

- **Leakage-wing baseline (edge 3.5 + fixed `const` order).** Fires on every
  fixture at a rate that tracks line density — 0.07 (2638, sparse gaussian) →
  0.52 (1512) → 0.93–0.96 (655 / 363, dense). The selected order is **`const`
  on 100% of fired windows across all seven fixtures** (linear was never
  chosen), so the fixed-const default is not a 2638 artefact. Tier-1 health is
  preserved where the baseline fires heavily (655 bulk χ²ᵣ med 3.08, 1512 1.18),
  i.e. evidence-gated firing is not harmful cross-fixture. **Validated —
  unchanged.**
- **Shape-aware classifier (`recommend_shape`).** Correct on all four
  ground-truth fixtures: 1512 / 1019 / 655 → lorentzian (exp vote 0.85 / 0.98 /
  0.95), 2638 → gaussian (gauss 0.68). The three MTBE fixtures without truth
  (360 / 363 / 1231) classify with sensible margins (a different molecule, so a
  non-Lorentzian lean is plausible, not a misfire). This closes the deferred
  settings-backfill cross-fixture-classifier follow-up. **Validated.**
- **`tau.fit_tau_min_snr` (50).** The tau-free rate rises monotonically with
  fixture SNR (1512 26 % free → 2638 43 % → 655 93 %), and **no bright window is
  ever wrongly held** (`held ≥ floor` ≈ 0 everywhere). The 50 threshold routes
  sanely across the SNR span. (Caveat: the audit's per-window SNR proxy is the
  max *fitted-peak* SNR, which undercounts the gating SNR, so the "free below
  floor" bucket is a proxy artefact, not a mis-route.) **Validated — unchanged.**
- **`conservative.weak_window_snr_threshold` (10).** The weak-window regime is
  exercised at a rate that scales with SNR (1512 0.84 → 655 0.39) and is never
  degenerate (no fixture at 0 or 1), so the 10 threshold is not mis-scaled for
  this instrument family. **Validated — unchanged.**
- **`rescue.snr_threshold` / `prominence_threshold` (2.5 / 2.0).** Rescue does
  meaningful, bounded work on every fixture (accept rate 0.4–0.7; limit-cycle
  rounds ≤ 11 % of rounds, absorbed by the merge). No pathological all-fire or
  no-fire. **Validated — unchanged.**
- **`thaw.residual_edge_threshold` (8).** The trigger surface is sane (21–118
  windows per fixture carry edge-coherence ≥ 8), but **thaw acceptance is ~0
  across all seven fixtures** (0 accepted on six, 1 on 1231) — the same
  near-dormant behaviour seen on 2638 now that the leakage-wing baseline covers
  bulk leakage. The edge threshold is not mistuned; whether the thaw step still
  earns its keep is a separate "does-it-fire" question (analogous to the ε
  retirement), flagged as a follow-up, not a #3 default change. **Validated —
  unchanged.**

**Stage 2 / 2b knobs** ride on the same builds: the scatter σ(f) and Stage 2b
STFT τ_maj + classifier produce the sane per-fixture inputs the above metrics
depend on (the classifier audit and the healthy Tier-1 distributions are the
direct evidence); no per-fixture retune indicated.

**Flagged anomaly (not a knob).** **363** (MTBE, no ground truth) fits poorly —
Tier-1 pass 0.19, χ²ᵣ median 28 — with structured residuals over a dense
high-amplitude forest (e.g. window 61: K=6, χ²ᵣ 9.5). Its *knob* behaviour is
normal (const-order baseline, rescue accept 0.73, classifier gaussian); the high
χ²ᵣ is a fixture-level lineshape/blend-fidelity problem (candidate causes:
gaussian-shape misfit on MTBE, spur contamination, unresolved blends),
independent of the audited constants. Recommend a separate investigation; out of
scope for the #3 knob audit.

Visual inspection set (random 8-window sample per fixture, seed 1234):
`scratch/stage5-validation/<id>/` with a roll-up `INDEX.md`, generated by the
generalized `scripts/development/stage5-validation/generate_validation.py`
(`--fixture <id> --random-sample N --seed S`).
