...
def load_blackchirp_experiment(
    ...
    return FTMWData(fid=fid, experiment_id=experiment_id, metadata=metadata)
...
def load_blackchirp_fid(experiment_path: Union[str, Path], fid_index: int = 0) -> FID:
    ...
    if fid_index >= len(fidparams_df):
        raise ValueError(
            f"FID index {fid_index} not found. Available indices: 0-{len(fidparams_df)-1}"
        )
...
def _load_blackchirp_metadata(experiment_path: Path) -> Dict[str, Any]:
    ...
    if header_file.exists():
        try:
            ...
            metadata["header"] = (
                header_df.to_dict("records")[0] if len(header_df) > 0 else {}
            )
        except Exception:
            pass
