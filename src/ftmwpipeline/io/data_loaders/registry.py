...
class FormatRegistry:
    ...
    def register_loader(self, format_name: str, loader_class: Type[BaseLoader]) -> None:
        ...
        try:
            ...
            logger.debug(f"Registered loader for format '{format_name}': {loader_class.__name__}")
        except Exception as e:
            logger.error(f"Failed to instantiate loader for format '{format_name}': {e}")
            # Remove from registry if instantiation fails
            if format_name in self._loaders:
                del self._loaders[format_name]
            raise LoaderError(f"Failed to register loader for format '{format_name}': {e}") from e
    ...
    def validate_source(self, source_path: Union[str, Path], 
                       format_name: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def load_fid(self, source_path: Union[str, Path], 
                 format_name: Optional[str] = None, **kwargs) -> 'FID':
        """
        ...
        **kwargs
        ...
        try:
            ...
            raise LoaderError(f"Failed to load data with format '{format_name}': {e}") from e
