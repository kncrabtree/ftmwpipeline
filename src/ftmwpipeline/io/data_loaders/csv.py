...
class CSVLoader(BaseLoader):
        ...
        try:
            ...
            try:
                ...
                if not pd.api.types.is_numeric_dtype(first_col):
                    result['errors'].append("CSV data does not appear to be numeric voltage values")
                    return result
    ...
    def load_fid(self, source_path: Union[str, Path], 
                 spacing_us: float, probe_freq_mhz: float,
                 sideband: str = 'upper', shots: int = 1, **kwargs) -> 'FID':
        """
