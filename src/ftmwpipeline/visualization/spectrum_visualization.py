...
def plot_complex_ft(complex_ft: ComplexFT, 
    ...
    **kwargs
    ...
    if backend == 'plotly':
        return _plot_complex_ft_plotly(freq, magnitude, real_part, imag_part, title, 
                                     fid, preprocessed_fid, show_raw_fid, show_preprocessed_fid, **kwargs)
    else:
        return _plot_complex_ft_matplotlib(freq, magnitude, real_part, imag_part, title, 
                                          figsize, interactive, fid, preprocessed_fid, 
                                          show_raw_fid, show_preprocessed_fid, **kwargs)


def _plot_complex_ft_plotly(freq: np.ndarray, magnitude: np.ndarray, 
                           real_part: np.ndarray, imag_part: np.ndarray, 
                           title: str, fid, preprocessed_fid, 
                           show_raw_fid: bool, show_preprocessed_fid: bool, **kwargs):
    ...
    if show_raw_fid:
        ...
        if hasattr(fid, 'processing') and fid.processing:
            ...
            if fid.processing.start_us is not None:
                fig.add_shape(
                    type="line",
                    x0=fid.processing.start_us, x1=fid.processing.start_us,
            ...
            if fid.processing.end_us is not None:
                fig.add_shape(
                    type="line", 
                    x0=fid.processing.end_us, x1=fid.processing.end_us,
                    y0=y_range[0], y1=y_range[1],
                    line=dict(color="red", width=2, dash="dash"),
                    row=current_row, col=1
                )
    ...
    fig.update_xaxes(title_text="Frequency (MHz)", showgrid=True, row=current_row, col=1)
...
def _plot_complex_ft_matplotlib(freq: np.ndarray, magnitude: np.ndarray,
                               ...
                               interactive: bool, fid, preprocessed_fid,
                               show_raw_fid: bool, show_preprocessed_fid: bool, **kwargs):
    ...
    if need_fid_panels:
        ...
        if show_raw_fid:
            ...
            if hasattr(fid, 'processing') and fid.processing:
                if fid.processing.start_us is not None:
                    ax_raw.axvline(fid.processing.start_us, color='r', linestyle='--', 
                                 linewidth=2, label=f'Start: {fid.processing.start_us:.1f} μs')
                if fid.processing.end_us is not None:
                    ax_raw.axvline(fid.processing.end_us, color='r', linestyle='--', 
                                 linewidth=2, label=f'End: {fid.processing.end_us:.1f} μs')
            ax_raw.legend()
        ...
        if show_preprocessed_fid:
            ...
            time_us = np.arange(len(preprocessed_fid.data)) * preprocessed_fid.spacing * 1e6
            ax_preproc.plot(time_us, preprocessed_fid.data, 'm-', linewidth=1, label='Preprocessed FID')
