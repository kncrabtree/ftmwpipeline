...
def plot_noise_estimation(
    ...
    if backend == "plotly":
        return _plot_noise_estimation_plotly(
            frequencies, magnitudes, noise_result, y_max_factor, 
            title, show_bin_boundaries, show_noise_points
        )
    else:
        return _plot_noise_estimation_matplotlib(
            frequencies, magnitudes, noise_result, y_max_factor, 
            figsize, title, show_bin_boundaries, show_noise_points
        )
...
def _plot_noise_estimation_matplotlib(
    ...
    fig.suptitle(title or "Noise Estimation Diagnostics", fontsize=14, fontweight='bold')
    ...
    if show_noise_points:
        ...
        ax.plot(freq_mhz, noise_magnitudes, color='orange', alpha=0.7, label='Noise Points')
    
    # Plot RMS noise estimate and multiples
    ax.plot(freq_mhz, noise_result.rms_noise, 'g-', linewidth=2, label='RMS Noise')
    ax.plot(freq_mhz, 3 * noise_result.rms_noise, 'g--', linewidth=1, alpha=0.7, label='3×RMS')
    ax.plot(freq_mhz, 5 * noise_result.rms_noise, 'g:', linewidth=1, alpha=0.7, label='5×RMS')
    ...
    if show_bin_boundaries and 'bin_edges' in noise_result.bin_info:
        ...
        if len(bin_edges) > 50:
            ...
            ax.text(0.02, 0.98, f'Showing {len(bin_edges_to_show)}/{len(bin_edges)} bin edges', 
        ...
        for edge in bin_edges_to_show:
            if 0 <= edge < len(frequencies):
                ax.axvline(freq_mhz[edge], color='red', linestyle=':', alpha=0.6, linewidth=0.5)
...
def _plot_noise_estimation_plotly(
    ...
    fig.add_trace(
        go.Scatter(x=freq_mhz, y=magnitudes, mode='lines', name='Spectrum', 
    ...
    if show_noise_points:
        ...
        fig.add_trace(
            go.Scatter(x=noise_freq, y=noise_mag, mode='markers', name='Noise Points',
    ...
    fig.add_trace(
        go.Scatter(x=freq_mhz, y=magnitudes, mode='lines', name='Spectrum', 
                  line=dict(color='blue', width=1), opacity=0.7, showlegend=False),
        row=1, col=2
    )
    ...
    if show_bin_boundaries and 'bin_edges' in noise_result.bin_info:
        ...
        for edge in bin_edges_to_show:
            if 0 <= edge < len(frequencies):
                fig.add_vline(
                    x=freq_mhz[edge], line_dash="dot", line_color="red",
                    opacity=0.8, line_width=1, row=1, col=2
                )
    ...
    fig.add_trace(
        go.Scatter(x=freq_mhz, y=noise_result.rms_noise, mode='lines', name='RMS Noise',
                  line=dict(color='green', width=2), fill='tozeroy', fillcolor='rgba(0,255,0,0.3)'),
        row=2, col=1
    )
    ...
    # Update layout
    fig.update_layout(
        title=title or "Noise Estimation Diagnostics",
        height=800,
        showlegend=True
    )
